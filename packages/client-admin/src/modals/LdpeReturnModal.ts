...
import {CodeProc, InputLdpeReturn, MainDbContext, StockProc} from "@sample/main-database";
      ...
      <sd-busy-container [busy]="busyCount > 0">
        <sd-sheet [id]="'combination-input-return-modal'" [items]="items" [trackBy]="trackByIdFn">
          ...
          <sd-sheet-column [header]="'반납일'">
            <ng-template #item let-item="item">
              <sd-textfield [(value)]="item.returnDate" [type]="'date'" [required]="true"></sd-textfield>
            </ng-template>
          </sd-sheet-column>
...
export class LdpeReturnModal extends SdModalBase<{ combinationId: number; productionInstructionId: number }, any> {
  public filter: {
  ...
  public items: ILdpeInputReturnModalVM[] = [];
  public orgItems: ILdpeInputReturnModalVM[] = [];
  ...
  public async sdOnOpen(param: { combinationId: number; productionInstructionId: number }): Promise<void> {
    this.busyCount++;

    this.filter.combinationId = param.combinationId;
    this.filter.productionInstructionId = param.productionInstructionId;
  ...
  public onRemoveItemButtonClick(item: ILdpeInputReturnModalVM): void {
  ...
  public async goodBuildGoodSearchModalOpenButtonClick(item: ILdpeInputReturnModalVM): Promise<void> {
  ...
  private async _save(): Promise<void> {
    ...
    if (!diffTargets || diffTargets.length < 1) {
      ...
      await this._orm.connectAsync(MainDbContext, async db => {
        for (const diff of diffs) {
          if (!diff.target!.id) {
            ...
            const newInputReturn = await db.inputLdpeReturn
              .insertAsync({
                companyId: this._appData.authInfo!.companyId,
                mixingProcessId: this.lastFilter!.combinationId!,
                productionInstructionId: this.filter.productionInstructionId!,
  ...
  private _getSearchQueryable(db: MainDbContext): Queryable<InputLdpeReturn> {
    return db.inputLdpeReturn
      .where(item => [
        sorm.equal(item.companyId, this._appData.authInfo!.companyId),
...
interface ILdpeInputReturnModalVM {
  id: number | undefined;
