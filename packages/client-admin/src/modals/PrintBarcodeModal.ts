      ...
      <sd-pane>
        <div>
          <sd-form [inline]="true" style="margin-top: 10px; margin-bottom: 10px; margin-left: 10px;">
            ...
            <br>
            <sd-form-item [label]="'생산일'" *ngIf="items && items.type === '생산'">
              <sd-textfield [(value)]="items.planDate" [type]="'date'"></sd-textfield>
            </sd-form-item>
...
export class PrintBarcodeModal extends SdModalBase<{ type?: string }, any> {
  public filter: {
    goodId?: number;
  } = {};
  ...
  public async onPrintButtonClick(): Promise<void> {
    ...
    if (this.items!.type === "생산" && !this.items!.planDate) {
      this._toast.danger("생산 예정일을 선택해 주세요.");
      return;
    }
